...
//        String msg = "题目的种类是" + antiMessageReq.getQuestionCategory() + ", 请根据题目内容对用户的能力进行分析，并给出数据用做echart图表展示(Echart的数据放在最后，并且是JSON格式，不用markdown的形式)，内容是:" + antiMessageReq.getQuestionContent() + "这是模板" + example;
        String msg = "题目的种类是" + antiMessageReq.getQuestionCategory() + ", 请根据题目内容对用户的能力进行分析，并给出数据用做echart的pie图表展示用户的优缺点(Echart的数据放在最后，并且是JSON格式, 不要在线的, 所有的 key 和 value 都要用双引号), 并以Markdown的形式，内容是:" + antiMessageReq.getQuestionContent() ;
